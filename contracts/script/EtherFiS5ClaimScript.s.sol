...
contract EtherFiS5ClaimScript is Script {
    ...
    bytes32 public constant REWARD_ROOT = 0x893c5744ddc0a47a6b8128595ca05d4d51e1fc9a7cc0e0200a1b4ee04def48d3;
    bytes32 public constant STAKING_ROOT = 0x476315c90a229b5fe8fbe22b1ad06a3920cd102edcd0ffb11173a27f8e123822;
    ...
    function run() public {
        ...
        Vars memory v = Vars("", address(0), 0, "", 0, 0, 0, "", "", "", "", "", "", "", 0, 0);

        v.projectRoot = vm.projectRoot();
        v.claimPath = string.concat(v.projectRoot, "/js-scripts/etherFiS4Rewards/KingRewards_6_17_25.json");
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            v.stakingRewards = 0;
            uint256 j = 0;
            console2.log("before key exists");
            if (vm.keyExists(v.claims, v.amountPath)) {
                console2.log("after key exists");
                if (vm.keyExists(v.claims, v.historicalRewardsPath)) {
                    ...
                    console2.log("entering while");
                    if(v.userAdd == address(0x028027B803c91d3e438E71e44f1ACcfbA4785df1) || v.userAdd == address(0xA9F1214d1E2b8BF263980F0A16eC2ACc9A2F2562)){
                        console2.log("this happened");
                    } else {
                        console2.log("no match");
                        while (true) {
                            console2.log("trying to parse forever");
                            try vm.parseJsonBytes32(`1`
                        ...
                        console2.log("out of while");
                    }
                }

                bytes32[] memory proof = vm.parseJsonBytes32Array(v.claims, v.proofPath);
                v.prevClaimed = lrt2Claim.cumulativeClaimed(v.userAdd);
                if(v.amount > v.prevClaimed){
                    lrt2Claim.claim(v.userAdd, v.amount, v.root, proof);
                } else {
                    console2.log(v.userAdd);
                    console2.log("nothing to claim");
                    console2.log("");
                    continue;
                }
                ...
                // if there are not staking rewards / previous balances, all rewards to vault
                if (v.stakingRewards == 0) {
                    ...
                    // if there is a yield transfer, array is 2 length
                    // otherwise there are only rewards, array is 1 length
                    if(v.stakingRewards >= v.prevClaimed){
                        transferCalls = new RumpelModule.Call[](2);
                    } else {
                        transferCalls = new RumpelModule.Call[](1);
                    }

                    // override reward amount if amount > bal
                    uint256 bal = lrt2.balanceOf(v.userAdd);
                    if(v.rewardAmount > bal) {
                        console2.log("rewards is greater than bal");
                        console2.log(v.rewardAmount);
                        console2.log(bal);
                        v.rewardAmount = bal;
                    }
                    ...
                    // transfer yield to safe owner
                    if(v.stakingRewards >= v.prevClaimed){
                        transferCalls[1] = RumpelModule.Call({
                            safe: ISafe(v.userAdd),
                            to: MAINNET_LRT2,
                            data: abi.encodeWithSelector(ERC20.transfer.selector, owners[0], v.stakingRewards - v.prevClaimed),
                            operation: Enum.Operation.Call
                        });
                    }
                }
                ...
                // console2.log(v.userAdd);
                console2.log(v.amount - v.prevClaimed, " - total claim");
                console2.log(v.rewardAmount, " - reward amount");
                console2.log(v.stakingRewards > v.prevClaimed ? v.stakingRewards - v.prevClaimed : 0, " - staking rewards");
                ...
                console2.log();
                // require(
                //     (lrt2.balanceOf(owners[0]) - v.ownerBalanceBefore) == (v.stakingRewards - v.prevClaimed), "ERROR: Owner Delta is Wrong"
                // );
            }
        }
        vm.stopBroadcast();
