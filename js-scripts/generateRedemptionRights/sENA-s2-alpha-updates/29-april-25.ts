...
import mergedDistribution01Apr25 from "../out/merged-distribution-01Apr25.json";
import fs from "fs";

const NEW_SENA = 20374030768000000000000n;

const updateRedemptionRights = () => {
  // Calculate total existing rights
  const total = Object.values(
    mergedDistribution01Apr25.redemptionRights
  ...
  for (const [address, rights] of Object.entries(
    ...
    mergedDistribution01Apr25.redemptionRights[address][tokenId].amount =
  ...
  const pTokenLeaves = Object.entries(
    mergedDistribution01Apr25.pTokens
  ...
  // Update root
  mergedDistribution01Apr25.root = tree.getHexRoot();
  ...
  for (const [address, rights] of Object.entries(
    ...
    for (const [pointsId, data] of Object.entries(rights)) {
      ...
      mergedDistribution01Apr25.redemptionRights[address][pointsId].proof =
  ...
  for (const [address, pointsData] of Object.entries(
    ...
    for (const [pointsId, data] of Object.entries(pointsData)) {
      ...
      mergedDistribution01Apr25.pTokens[address][pointsId].proof =
        tree.getHexProof(leaf);
    }
  }
  ...
  fs.writeFileSync(
    "./js-scripts/generateRedemptionRights/out/merged-distribution-29Apr25.json",
    JSON.stringify(mergedDistribution01Apr25, null, 2)
  );
};
