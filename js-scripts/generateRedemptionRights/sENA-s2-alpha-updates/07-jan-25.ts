...
import mergedDistribution03Dec24 from "../out/merged-distribution-03Dec24.json";
import fs from "fs";
...
const updateRedemptionRights = () => {
  ...
  const total = Object.values(
    mergedDistribution03Dec24.redemptionRights
  ...
  for (const [address, rights] of Object.entries(
    ...
    mergedDistribution03Dec24.redemptionRights[address][tokenId].amount =
  ...
  const pTokenLeaves = Object.entries(
    mergedDistribution03Dec24.pTokens
  ...
  // Update root
  mergedDistribution03Dec24.root = tree.getHexRoot();
  ...
  for (const [address, rights] of Object.entries(
    ...
    for (const [pointsId, data] of Object.entries(rights)) {
      ...
      mergedDistribution03Dec24.redemptionRights[address][pointsId].proof =
  ...
  for (const [address, pointsData] of Object.entries(
    ...
    for (const [pointsId, data] of Object.entries(pointsData)) {
      ...
      mergedDistribution03Dec24.pTokens[address][pointsId].proof =
        tree.getHexProof(leaf);
    }
  }
  ...
  fs.writeFileSync(
    "./js-scripts/generateRedemptionRights/out/merged-distribution-07Jan25.json",
    JSON.stringify(mergedDistribution03Dec24, null, 2)
  );
};
