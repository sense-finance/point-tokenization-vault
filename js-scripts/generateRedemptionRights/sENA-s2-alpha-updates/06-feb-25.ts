...
import mergedDistribution07Jan25 from "../out/merged-distribution-07Jan25.json";
import fs from "fs";
...
const updateRedemptionRights = () => {
  ...
  const total = Object.values(
    mergedDistribution07Jan25.redemptionRights
  ...
  for (const [address, rights] of Object.entries(
    ...
    mergedDistribution07Jan25.redemptionRights[address][tokenId].amount =
  ...
  const pTokenLeaves = Object.entries(
    mergedDistribution07Jan25.pTokens
  ...
  // Update root
  mergedDistribution07Jan25.root = tree.getHexRoot();
  ...
  for (const [address, rights] of Object.entries(
    ...
    for (const [pointsId, data] of Object.entries(rights)) {
      ...
      mergedDistribution07Jan25.redemptionRights[address][pointsId].proof =
  ...
  for (const [address, pointsData] of Object.entries(
    ...
    for (const [pointsId, data] of Object.entries(pointsData)) {
      ...
      mergedDistribution07Jan25.pTokens[address][pointsId].proof =
        tree.getHexProof(leaf);
    }
  }
  ...
  fs.writeFileSync(
    "./js-scripts/generateRedemptionRights/out/merged-distribution-06Feb25.json",
    JSON.stringify(mergedDistribution07Jan25, null, 2)
  );
};
